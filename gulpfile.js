...
gulp.task('less-build', function () {
    gulp
        .src('build/less/*/*.less') //该任务针对的文件
        .pipe(less()) //该任务调用的模块
        .pipe(minifyCss()) //对生成后的css进行压缩
        .pipe(gulp.dest(outpath + '/css')) //将会在build/css下生成对应css
});
//监听js文件、合并、压缩
...
gulp.task('connect', function () {
    connect.server({
        root: './', ip: '10.101.62.65', port: '8081', //可以修改启动端口号
        livereload: true
    });
});
...
gulp.task('webserver', function () {
    gulp
        .src('build')
        .pipe(webserver({open: true, port: 8081}));
});
...
gulp.task('html', function () {
    gulp
        .watch('build/html/*/*.html', function () {
            gulp
                .src('build/html/*/*.html')
                .pipe(connect.reload());
        })
...
function bytediffFormatter(data) {
    let difference = (data.savings > 0)
            ? ' smaller.'
            : ' larger.',
        result = data.savings > 0
            ? ('and is ' + $.util.colors.yellow.bold(~~ ((1 - data.percent) * 100) + '%') + difference)
            : "";
    return `${$
        .util
        .colors
        .cyan
        .bold(data.fileName)} from ${$
        .util
        .colors
        .yellow
        .bold((data.startSize / 1000).toFixed(2))} kB to ${$
        .util
        .colors
        .yellow
        .bold((data.endSize / 1000).toFixed(2))} kB ${result}`;
}