...
// NOTICE: substring is faster than negative slice and RegExp
    function normalize(path) {
        ...
        return (path.substring(last - 2) === ".js" || path.substring(last - 4) === ".ashx" ||
        path.indexOf("?") > 0 ||
...
// Define a module
    Module.define = function (id, deps, factory) {
        ...
        var remoteManifest = (data.localcache && data.localcache.manifest) || {} 
          if(remoteManifest){
                //  var _id = seajs.resolve(id);     
               if(remoteManifest[id] && isFunction(factory) && localManifest[seajs.resolve(id)]!=remoteManifest[id]){
                          
                    var resp;

                    if (argsLen === 1) {
                       
                        resp = "define("+factory.toString()+");";
                    }
                    else if (argsLen === 2) {
                        ...
                        if (isArray(id)) {
                            ...
                            deps = '['+deps.toString()+']';
                            resp = "define("+deps+","+factory.toString()+");";

                        }
                        // define(id, factory)
                        else {
                         
                            resp = "define('"+id+"',"+factory.toString()+");";
                        }
                    }
                     storage.set(seajs.resolve(id), resp)
