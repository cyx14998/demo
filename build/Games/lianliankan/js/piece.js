...
Piece.prototype = {
    ...
    setSelected: function () {
        if (this.flashCount++ % 2 == 0) {
            ...
            //if (this.selected != null) { this.dom.removeChild(this.selected); }
    ...
    isNear: function (piece) {
        var a = (Math.abs(piece.x - this.x) == 1) && (piece.y == this.y) || (Math.abs(piece.y - this.y) == 1) && (piece.x == this.x);
        return a;
    },
    // 直线
            ...
            for (var i = this.min(this.y, piece.y) + 1; i < this.max(this.y, piece.y); i++) {
                ...
                if (this.game.pieceMap.get(piece.x + "," + i).isPass()) {
                    ...
                    this
                        .game
                        .trackList
                        .push(this.game.pieceMap.get(piece.x + "," + i));
                    continue;
                } else {
                    a = false;
                    this.game.trackList = [];
            ...
            for (var i = this.min(this.x, piece.x) + 1; i < this.max(this.x, piece.x); i++) {
                ...
                if (this.game.pieceMap.get(i + "," + piece.y).isPass()) {
                    ...
                    this
                        .game
                        .trackList
                        .push(this.game.pieceMap.get(i + "," + piece.y));
                    continue;
                } else {
                    b = false
                    this.game.trackList = [];
    ...
    isReach1: function (piece) {
        ...
        var corner_2 = this
            ...
            .get(piece.x + "," + this.y);
        var _this = this;
        if ((_this.isStraightReach(corner_1)) && (corner_1.isStraightReach(piece)) && corner_1.isPass()) {
        ...
        if ((_this.isStraightReach(corner_2)) && (corner_2.isStraightReach(piece)) && corner_2.isPass()) {
    ...
    isReach2: function (piece) {
        ...
        for (var i = this.x + 1; i < 17; i++) {
            if (!this.game.pieceMap.get(i + "," + this.y).isPass()) {
                this.game.trackList = [];
                break;
            } else if (this.game.pieceMap.get(i + "," + this.y).isReach(piece) && this.game.pieceMap.get(i + "," + this.y).isPass()) {
                this
                    .game
                    .trackList
                    .push(this.game.pieceMap.get(i + "," + this.y));
                return true;
            }
        }
        // 沿x轴搜索
        ...
        for (var i = this.y - 1; i >= 0; i--) {
            if (!this.game.pieceMap.get(this.x + "," + i).isPass()) {
                this.game.trackList = [];
                break;
            } else if (this.game.pieceMap.get(this.x + "," + i).isReach(piece) && this.game.pieceMap.get(this.x + "," + i).isPass()) {
                this
                    .game
                    .trackList
                    .push(this.game.pieceMap.get(this.x + "," + i));
                return true;
            }
        }
        // 沿y轴正向搜索
