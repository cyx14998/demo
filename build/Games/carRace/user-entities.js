var Car = function (v, style, img, speed) {
    ...
    this._update = function () {
        ...
        // this.position.y += (MainApp.nowTime - MainApp.startTime) * this.speed.y /
        // 1000; this.position.x += (MainApp.nowTime - MainApp.startTime) * this.speed.x
        // / 1000;

        if (this.position.y > 480) {
            this.position.y = window
                .util
                .random(-240, 0);
            this.speed = new Vector(0, Math.ceil(window.util.random(80, 180)));
        }
...
var GameScore = function (s, p) {
    ...
    this.score = s;
    var text = new TextEntityObject(s, p, {
        fillStyle: '#fff',
        font: 'bold 32px 微软雅黑',
        'textBaseline': 'top'
    }, 100, 35);
