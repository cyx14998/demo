                            ...
                            if (manitemlist.length == 0) {
                                ...
                                for (var t = 0, tLen = manitemlist.length; t < tLen ; t++) {
                                    ...
                                    if (PageFlag == 1) {
                                        ...
                                        if (mRemark) {
                                            remarkHtml = '<input class="getval" type="text" placeholder="不通过请填写不通过原因" size="50" value="' + mRemark + '"/>';
                                        }
                                        else {
                                            remarkHtml = '<input class="getval" type="text" placeholder="不通过请填写不通过原因" size="50" />';
                                        }

                                        materialHtml += '<tr><td>' + mName + '</td>' + '<td>' + EnclosureStr + '</td>' + '<td class="status" MaterialId="' + mNd + '" UserId="' + UserId + '" status="' + mstatusvalue + '" mstatus="' + mstatus + '" currentPageFlag="' + currentPageFlag + '">' + mstatus + '</td>' + '<td>' + passHtml + '</td>' +
                                            '<td class="remark">' + remarkHtml + '</td></tr>';
                                    }
                                }
                                if (PageFlag == 1) {
                                    if (currentPageFlag > 1) {
                                    ...
                                    Downtourlist += '<div class="item none"index=' + i + '><div class="listbox"><div class="cont_details"><table cellspacing="0"><thead><tr><th class="w_10">材料内容</th>'
                                        + '<th class="w_245">附件列表</th><th class="w_10">材料状态</th><th class="w_20">' + passHeadHtml + '</th>'
                                        + '<th class="w_35">备注</th></tr></thead><tbody>' + materialHtml + '</tbody></table></div></div></div>';
                                }
                            }
...
function orderBtn(PageFlag, IsDetail) {
    var orderserialid = "",
        serialid = "",
    ...
    if (PageFlag == 1) {
        ...
        var objs = $($(".outline")[1]).find(".Downtour .status");
        ...
        if (IsAllActionTrue && !ReadySendDate) {
            $.tip({
                title: '操作确认',
